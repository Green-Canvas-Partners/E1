...
for day in range(18):
    ...
    df = df[['Date','Momentum_30_30','Stock','Returns']].sort_values('Date').reset_index(drop=True)

    dt = np.array(df[(df.Stock=='SPY')].Date)
    dict = {}
    for i in dt:
        tmp = df[df.Date == i].copy()
        sorted_tmp = tmp.sort_values('Momentum_30_30', ascending=False).drop_duplicates()

        # Only include stocks with positive momentum in the top 10
        positive_momentum_stocks = sorted_tmp[sorted_tmp['Momentum_30_30'] > 0]
        if len(positive_momentum_stocks) >= 32:
            dict[i] = positive_momentum_stocks.head(10).Stock.values
        else:
            dict[i] = np.array([])
...
for days in range(18):
    # print(days)
    momentum = data[days]
    momentum = momentum[['Date','Momentum_30_30','Stock','Returns']].sort_values('Date').reset_index(drop=True)
...
for days in range(18):
    ...
    def weighted_return(row, tickers):
        ...
        weights = exponential_weights(10, 0.5)
        # print(weights)
